...
import { Users, GraduationCap, Heart, BookOpen, TrendingUp, Clock, CheckSquare, AlertCircle, Edit3, Eye } from 'lucide-react';
import { LineChart, Line, XAxis, YAxis, Tooltip, ResponsiveContainer, CartesianGrid } from 'recharts';
import { analyticsAPI, attendanceAPI } from '../services/api';
import { useAuth } from '../contexts/AuthContext';
import { StatCard, LoadingPage, CategoryBadge, StatusBadge } from '../components/common';
  ...
  return (
    <div>
      ...
      <div className="grid-4" style={{ marginBottom: 24 }}>
        <StatCard label="Total Students" value={stats.students?.total} icon={Users} color="#3b82f6" subtitle={`${stats.students?.beginner} Beginners · ${stats.students?.inter} Inter`} />
      ...
      <div className="grid-2" style={{ marginBottom: 24 }}>
        <div className="card">
          <div className="card-header">
            ...
            <span style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)' }}>{format(new Date(), 'EEEE, MMMM d')}</span>
          </div>
          <div className="card-body">
            <div style={{ display: 'flex', gap: 16, marginBottom: 20 }}>
              <div style={{ flex: 1, background: '#d1fae510', border: '1px solid #10b98130', borderRadius: 12, padding: 16, textAlign: 'center' }}>
                <div style={{ fontSize: '2rem', fontWeight: 800, color: '#10b981' }}>{stats.today?.presentCount ?? '—'}</div>
                <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: 4 }}>Present</div>
              </div>
              <div style={{ flex: 1, background: '#dbeafe10', border: '1px solid #3b82f630', borderRadius: 12, padding: 16, textAlign: 'center' }}>
                <div style={{ fontSize: '2rem', fontWeight: 800, color: '#3b82f6' }}>{stats.today?.attendanceRate ?? 0}%</div>
                <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginTop: 4 }}>Rate</div>
              </div>
            </div>
            ...
            {stats.today?.pendingClasses > 0 && (
              <div style={{ background: '#fef3c7', border: '1px solid #fbbf24', borderRadius: 10, padding: 12, marginBottom: 12 }}>
                <div style={{ fontSize: '0.8rem', fontWeight: 700, color: '#92400e', display: 'flex', alignItems: 'center', gap: 6 }}>
                  <AlertCircle size={14} /> {stats.today.pendingClasses} class{stats.today.pendingClasses > 1 ? 'es' : ''} pending attendance
                </div>
                {stats.today.pendingClassList?.map((c) => (
                  <div key={c._id} style={{ fontSize: '0.75rem', color: '#92400e', marginTop: 4, paddingLeft: 20 }}>• {c.name} ({c.category})</div>
                ))}
              </div>
            )}

            <button className="btn btn-secondary btn-sm" onClick={() => navigate('/attendance')} style={{ width: '100%', justifyContent: 'center' }}>
              ...
              <div>
                {[
                  ...
                  <div key={item.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid var(--color-border-light)' }}>
                    ...
                    <span className="badge" style={{ background: `${item.color}20`, color: item.color }}>{item.count} pending</span>
                  </div>
                ))}
                <button className="btn btn-primary btn-sm" onClick={() => navigate('/verification')} style={{ width: '100%', justifyContent: 'center', marginTop: 16 }}>
                  Review Queue →
                </button>
              </div>
            )}
        ...
        {[
          ...
          <div key={cat} className="card" style={{ textAlign: 'center', padding: 20 }}>
            ...
            <div style={{ fontSize: '0.75rem', color: 'var(--color-text-secondary)', marginTop: 8 }}>
              {stats.students?.total ? Math.round((count / stats.students.total) * 100) : 0}% of total
            </div>
          </div>
        ))}
      </div>
    ...
    <div>
      <div className="grid-2" style={{ marginBottom: 24 }}>
        <div className="card">
          ...
          <div className="card-body">
            <div style={{ fontSize: '0.875rem', color: 'var(--color-text-secondary)', marginBottom: 12 }}>{windowData?.message}</div>
            {windowData?.allowed && windowData?.minutesRemaining > 0 && (
              <div style={{ background: '#fef3c7', borderRadius: 10, padding: 12, textAlign: 'center' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 800, color: '#92400e' }}>{windowData.minutesRemaining} min</div>
        ...
        <div className="card">
          ...
          <div className="card-body" style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <button className="btn btn-primary" onClick={() => window.location.href = '/attendance/submit'} disabled={!windowData?.allowed}>
              ✏️ Mark Student Attendance {!windowData?.allowed && '(Window Closed)'}
            </button>
            <button className="btn btn-secondary" onClick={() => window.location.href = '/my-class'}>
              👥 View My Class
            </button>
          </div>
        </div>
  ...
  return (
    <div>
      <div className="grid-3">
        {[
          { label: 'Add Student', path: '/students', icon: Users, color: '#3b82f6', desc: 'Submit for approval' },
          { label: 'Add Teacher', path: '/teachers', icon: GraduationCap, color: '#8b5cf6', desc: 'Submit for approval' },
          { label: 'Add Volunteer', path: '/volunteers', icon: Heart, color: '#10b981', desc: 'Submit for approval' },
        ].map(({ label, path, icon: Icon, color, desc }) => (
          <motion.div key={label} whileHover={{ y: -3 }} className="card" style={{ cursor: 'pointer', padding: 24, textAlign: 'center' }} onClick={() => window.location.href = path}>
  ...
  return (
    <div>
      <div className="grid-4" style={{ marginBottom: 24 }}>
        ...
        <StatCard label="Teachers" value={stats.teachers?.total} icon={GraduationCap} color="#8b5cf6" />
        <StatCard label="Volunteers" value={stats.volunteers?.total} icon={Heart} color="#10b981" />
        <StatCard label="Attendance Rate" value={`${stats.today?.attendanceRate ?? 0}%`} icon={TrendingUp} color="#f59e0b" />
      </div>
      <div className="card">
        ...
        <div className="card-body" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
          {[
            { label: 'Daily Report', path: '/reports?type=daily', icon: Clock },
            { label: 'Full Year', path: '/reports?type=year', icon: TrendingUp },
            { label: 'Analytics', path: '/analytics', icon: Eye },
          ].map(({ label, path, icon: Icon }) => (
            <button key={label} onClick={() => navigate(path)} className="btn btn-secondary" style={{ justifyContent: 'center', flexDirection: 'column', height: 70, gap: 6 }}>
              <Icon size={18} />{label}
            </button>
          ))}
        </div>
    ...
    <div>
      <div className="page-header">
        ...
        <p className="page-subtitle">
          ...
          {user?.role === 'editor' && 'Submit and manage entries for approval'}
          {user?.role === 'viewer' && 'Read-only access to all data and reports'}
          {user?.role === 'teacher' && 'Manage your class attendance'}
        </p>
      </div>
      ...
      {user?.role === 'viewer' && <ViewerDashboard stats={data || {}} />}
    </div>
  );
}
