...
const { isWithinVBSSchedule } = require('../services/attendanceWindowService');
...
const submitTeacherAttendance = async (req, res, next) => {
  try {
    ...
    // records: [{ teacherId, status, arrivalTime, departureTime, remarks }]
    ...
    for (const r of records) {
      try {
        const existing = await TeacherAttendance.findOne({ date: attendanceDate, teacher: r.teacherId });

        if (existing) {
          // Editor cannot modify existing
          if (req.user.role === 'editor') {
            results.failed.push({ teacherId: r.teacherId, reason: 'Editors cannot modify existing attendance' });
            continue;
          }
          // Admin modification with audit trail
          const changes = [];
          if (existing.status !== r.status) changes.push({ entityId: r.teacherId, previousStatus: existing.status, newStatus: r.status });
          ...
          existing.modificationHistory.push({
            modifiedBy: req.user._id, modifiedByName: req.user.name,
            modifiedAt: new Date(), changes,
            reason: req.body.reason || 'Attendance updated',
          });
          await existing.save();
          results.updated.push({ teacherId: r.teacherId });
        } else {
          const record = await TeacherAttendance.create({
            date: attendanceDate, teacher: r.teacherId, vbsYear: scheduleCheck.vbsYear,
            status: r.status, arrivalTime: r.arrivalTime, departureTime: r.departureTime,
            remarks: r.remarks, markedBy: req.user._id, markedByName: req.user.name,
          });
...
const modifyTeacherAttendance = async (req, res, next) => {
  try {
    ...
    const changes = [{ entityId: record.teacher?._id, entityName: record.teacher?.name,
      previousStatus: record.status, newStatus: status || record.status,
      previousArrivalTime: record.arrivalTime, newArrivalTime: arrivalTime,
      previousDepartureTime: record.departureTime, newDepartureTime: departureTime,
    }];
    ...
    record.modificationHistory.push({
      modifiedBy: req.user._id, modifiedByName: req.user.name,
      modifiedAt: new Date(), changes, reason: reason || 'No reason specified',
    });
...
const submitVolunteerAttendance = async (req, res, next) => {
  try {
    ...
    for (const r of records) {
      try {
        const existing = await VolunteerAttendance.findOne({ date: attendanceDate, volunteer: r.volunteerId });

        if (existing) {
          if (req.user.role === 'editor') {
            results.failed.push({ volunteerId: r.volunteerId, reason: 'Editors cannot modify existing attendance' });
            continue;
          }
          existing.status = r.status;
          ...
          existing.modificationHistory.push({
            modifiedBy: req.user._id, modifiedByName: req.user.name,
            modifiedAt: new Date(), changes: [{ entityId: r.volunteerId, previousStatus: existing.status, newStatus: r.status }],
          ...
          await VolunteerAttendance.create({
            date: attendanceDate, volunteer: r.volunteerId, vbsYear: scheduleCheck.vbsYear,
            status: r.status, shift: r.shift, checkInTime: r.checkInTime,
            checkOutTime: r.checkOutTime, remarks: r.remarks,
            markedBy: req.user._id, markedByName: req.user.name,
          });
    ...
    res.status(201).json({
      success: true, message: `${results.created.length} created, ${results.updated.length} updated`,
      data: results,
    });
  } catch (err) { next(err); }
};
...
const modifyVolunteerAttendance = async (req, res, next) => {
  try {
    ...
    const changes = [{ entityId: record.volunteer?._id, entityName: record.volunteer?.name,
      previousStatus: record.status, newStatus: status || record.status,
      previousShift: record.shift, newShift: shift,
    }];
