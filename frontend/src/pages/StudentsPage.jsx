    ...
    <div className="modal-overlay" onClick={onClose}>
      <motion.div className="modal modal-lg" onClick={e => e.stopPropagation()}
        ...
        <div className="modal-footer">
          ...
          <button className="btn btn-primary" onClick={() => { if (validate()) onSuccess(form); }}>
...
function BulkAllocateModal({ isOpen, onClose, selectedStudents, classes, onSuccess }) {
  ...
  const [step, setStep] = useState(1); // 1: select class, 2: confirm
  ...
  const allCompatible = selectedStudents.every(s => GRADE_TO_CATEGORY[s.grade] === (classes.find(c => c._id === selectedClass)?.category));
...
function ClassRosterView({ classes, students, onAllocate, onBulkAllocate, isAdmin }) {
  ...
  const [unallocatedFilter, setUnallocatedFilter] = useState(false);
                    ...
                    {isAdmin && <th style={{ padding: '8px 12px', background: 'var(--color-bg)' }}>
                      <input type="checkbox"
                        ...
                        onChange={() => setSelectedIds(
                          selectedIds.length === filteredUnallocated.length ? [] : filteredUnallocated.map(s => s._id)
                        )} />
                    </th>}
                    <th style={{ padding: '8px 12px', fontSize: '0.68rem', color: 'var(--color-text-muted)', fontWeight: 700, textAlign: 'left', background: 'var(--color-bg)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Name</th>
                    <th style={{ padding: '8px 12px', fontSize: '0.68rem', color: 'var(--color-text-muted)', fontWeight: 700, textAlign: 'left', background: 'var(--color-bg)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Grade</th>
...
export default function StudentsPage() {
  ...
  const { data, isLoading, isFetching } = useQuery({
    queryKey: ['students', page, search, filterGrade, filterCategory, filterClass],
    queryFn: () => studentsAPI.getAll({
      page, limit: 50, search, grade: filterGrade,
      category: filterCategory, classAssigned: filterClass
    }),
    select: d => d.data,
  ...
  const { data: allStudentsData } = useQuery({
    queryKey: ['all-students-allocation'],
    queryFn: () => studentsAPI.getAll({ limit: 500 }),
    select: d => d.data?.data || [],
    enabled: viewMode === 'allocation' && isAdmin,
  });
  ...
  const handleAllocateSingle = (student, classId) => {
    if (classId !== undefined) {
      // Direct allocation from roster view
      allocateMutation.mutate({ studentId: student._id, classId });
    } else {
    ...
    <div>
      ...
      {viewMode === 'allocation' && isAdmin && (
        <ClassRosterView
          ...
          onBulkAllocate={(studs, classId) => {
            if (classId) {
              bulkAllocateMutation.mutate({ studentIds: studs.map(s => s._id), classId });
            }
          }}
          isAdmin={isAdmin}
        />
                            ...
                            {!isReadOnly && (
                              <td>
                                <div style={{ display: 'flex', gap: 4 }}>
                                  ...
                                  {isAdmin && (
                                    <button className="btn btn-ghost btn-icon btn-sm" onClick={() => window.confirm(`Delete "${s.name}"?`) && deleteMutation.mutate(s._id)} title="Delete" style={{ color: 'var(--color-danger)' }}>
                                      <Trash2 size={13} />
                                    </button>
                                  )}
                                </div>
      ...
      {allocateStudent && (
        <ClassAllocationModal
          ...
          onSuccess={(studentId, classId) => {
            if (classId) {
              allocateMutation.mutate({ studentId, classId });
            } else {
              updateMutation.mutate({ id: studentId, data: { classAssigned: null } });
              setAllocateStudent(null);
            }
          }}
        />
      ...
      {bulkAllocateStudents && (
        <BulkAllocateModal
          ...
          classes={classes}
          onSuccess={(studentIds, classId) => bulkAllocateMutation.mutate({ studentIds, classId })}
        />
      )}
    </div>
  );
