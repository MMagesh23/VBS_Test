...
const { notifyPendingVerification, notifyEntryApproved, notifyEntryRejected } = require('../services/notificationService');
...
const createTeacher = async (req, res, next) => {
  try {
    ...
    if (req.user.role === 'editor') {
      ...
      return res.status(201).json({ success: true, message: 'Teacher submitted for approval', data: staged, staged: true });
    }
...
const assignTeacherToClass = async (req, res, next) => {
  try {
    ...
    const teacher = await Teacher.findByIdAndUpdate(req.params.id, { classAssigned: classId }, { new: true })
      .populate('classAssigned', 'name category');
    if (!teacher) return res.status(404).json({ success: false, message: 'Teacher not found' });

    // Also update Class model to reference teacher
    await Class.findByIdAndUpdate(classId, { teacher: req.params.id });

    res.json({ success: true, message: `${teacher.name} assigned to ${cls.name}`, data: teacher });
  } catch (err) { next(err); }
};
...
const getStagingTeachers = async (req, res, next) => {
  try {
    ...
    const teachers = await StagingTeacher.find(filter).populate('createdBy', 'name userID').sort({ createdAt: -1 });
    res.json({ success: true, count: teachers.length, data: teachers });
  } catch (err) { next(err); }
};
...
const approveStagedTeacher = async (req, res, next) => {
  try {
    ...
    if (!staged) return res.status(404).json({ success: false, message: 'Staged teacher not found' });

    const teacher = await Teacher.create({ ...staged.toObject(), _id: undefined, createdBy: staged.createdBy._id, approvedBy: req.user._id, approvedAt: new Date() });
...
const createVolunteer = async (req, res, next) => {
  try {
    ...
    if (req.user.role === 'editor') {
      ...
      return res.status(201).json({ success: true, message: 'Volunteer submitted for approval', data: staged, staged: true });
    }
...
const updateVolunteer = async (req, res, next) => {
  try {
    const v = await Volunteer.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
    if (!v) return res.status(404).json({ success: false, message: 'Volunteer not found' });
...
const getStagingVolunteers = async (req, res, next) => {
  try {
    const filter = req.user.role === 'editor' ? { createdBy: req.user._id } : {};
    const volunteers = await StagingVolunteer.find(filter).populate('createdBy', 'name userID').sort({ createdAt: -1 });
    res.json({ success: true, count: volunteers.length, data: volunteers });
  } catch (err) { next(err); }
};
...
const approveStagedVolunteer = async (req, res, next) => {
  try {
    ...
    if (!staged) return res.status(404).json({ success: false, message: 'Not found' });
    const v = await Volunteer.create({ ...staged.toObject(), _id: undefined, createdBy: staged.createdBy._id, approvedBy: req.user._id, approvedAt: new Date() });
...
module.exports = {
  ...
  getStagingTeachers, approveStagedTeacher, rejectStagedTeacher,
  getVolunteers, getVolunteer, createVolunteer, updateVolunteer, deleteVolunteer,
  getStagingVolunteers, approveStagedVolunteer, rejectStagedVolunteer,
};
