...
const getStudents = async (req, res, next) => {
  try {
    ...
    if (req.user.role === 'teacher') {
      ...
      // Teachers don't need category/religion filters
...
const bulkAllocate = async (req, res, next) => {
  try {
    ...
    if (!studentIds?.length || !classId) return res.status(400).json({ success: false, message: 'Provide studentIds and classId' });
...
const approveStagedStudent = async (req, res, next) => {
  try {
    ...
    if (existing) {
      return res.status(400).json({ success: false, message: `Possible duplicate: "${existing.name}" (${existing.studentId}) already exists.`, data: { existingStudentId: existing.studentId } });
    }
    ...
    const student = await Student.create({
      ...
      sameAsContact,
      whatsappNumber: sameAsContact ? (overrides.contactNumber || staged.contactNumber) : (overrides.whatsappNumber || staged.whatsappNumber),
...
const bulkApproveStagedStudents = async (req, res, next) => {
  try {
    ...
    res.json({ success: true, message: `${results.approved.length} approved, ${results.failed.length} failed`, data: results });
  } catch (err) { next(err); }
};
