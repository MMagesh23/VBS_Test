...
const { checkAttendanceWindow, isWithinVBSSchedule } = require('../services/attendanceWindowService');
...
const getStudentAttendance = async (req, res, next) => {
  try {
    ...
    // Teacher: restrict to own class
    if (req.user.role === 'teacher') {
      const teacher = await Teacher.findOne({ user: req.user._id });
      if (!teacher?.classAssigned) return res.json({ success: true, data: [] });
  ...
  try {
    ...
    if (req.user.role === 'teacher') {
      ...
      if (!teacher || teacher.classAssigned?.toString() !== classId) {
        return res.status(403).json({ success: false, message: 'You can only submit attendance for your assigned class' });
      }
    ...
    res.status(201).json({ success: true, message: 'Attendance submitted successfully', data: attendance });
  } catch (err) {
    if (err.code === 11000) {
      return res.status(400).json({ success: false, message: 'Attendance already exists for this date and class' });
    }
    next(err);
  }
};
...
const modifyStudentAttendance = async (req, res, next) => {
  try {
    ...
    // changes: [{ studentId: ObjectId, newStatus: 'present'|'absent' }]
    if (!changes?.length) {
    ...
    for (const change of changes) {
      const record = attendance.records.find(
        (r) => r.student?._id?.toString() === change.studentId || r.student?.toString() === change.studentId
      );
      if (!record) continue;
...
// @desc    Get attendance window status
...
const getTodaySummary = async (req, res, next) => {
  try {
    ...
    const [submitted, classes] = await Promise.all([
      StudentAttendance.find({ date: today, ...yearFilter })
        .populate('class', 'name category')
        .populate('submittedBy', 'name'),
      Class.find({ ...yearFilter, isActive: true }),
    ]);
    ...
    const totalPresent = submitted.reduce((sum, s) => sum + s.records.filter((r) => r.status === 'present').length, 0);
    const totalAbsent = submitted.reduce((sum, s) => sum + s.records.filter((r) => r.status === 'absent').length, 0);
