...
const getDailyReport = async (req, res, next) => {
  try {
    ...
    const unsubmittedClasses = classes.filter(
      (c) => !submittedClassIds.includes(c._id.toString())
    );
        ...
        summary: {
          students: {
            ...
            rate: totalStudentPresent + totalStudentAbsent > 0
              ? Math.round((totalStudentPresent / (totalStudentPresent + totalStudentAbsent)) * 100)
              : 0,
          },
          teachers: {
...
const getTeacherReport = async (req, res, next) => {
  try {
    ...
    // Safe date diff with null guard
    const totalDays =
      settings?.dates?.startDate && settings?.dates?.endDate
        ? Math.round(
            (new Date(settings.dates.endDate) - new Date(settings.dates.startDate)) /
              (1000 * 60 * 60 * 24)
          ) + 1
        : 0;
    ...
    res.json({
      ...
      data: {
        ...
        submissions: {
          ...
          submissionRate:
            totalDays > 0
              ? Math.round((submissionHistory.length / totalDays) * 100)
              : 0,
        },
        ownAttendance: {
...
const getVolunteerReport = async (req, res, next) => {
  try {
    ...
    const settings = await Settings.findOne({ isActive: true });
    const attendance = await VolunteerAttendance.find({
      ...
      data: {
        ...
        attendance: {
          ...
          rate:
            totalDays > 0
              ? Math.round(
                  (attendance.filter((a) => ['present', 'halfDay'].includes(a.status)).length /
                    totalDays) *
                    100
                )
              : 0,
...
const getFullYearReport = async (req, res, next) => {
  try {
    ...
    const settings = await Settings.findOne(
      vbsYear ? { year: Number(vbsYear) } : { isActive: true }
    );
    if (!settings) {
      return res.status(404).json({ success: false, message: 'VBS year not found' });
    }

    const year = settings.year;

    const [
      students, teachers, volunteers, classes,
      allStudentAttendance, allTeacherAttendance, allVolunteerAttendance,
    ] = await Promise.all([
    ...
    const totalStudentRecords = allStudentAttendance.reduce(
      (sum, a) => sum + a.records.length, 0
    );
    const totalPresent = allStudentAttendance.reduce(
      (sum, a) => sum + a.records.filter((r) => r.status === 'present').length, 0
    );
      ...
      data: {
        ...
        summary: {
          ...
          attendance: {
            ...
            teachers: {
              total: allTeacherAttendance.length,
              present: allTeacherAttendance.filter((a) => a.status === 'present').length,
            },
            volunteers: {
              total: allVolunteerAttendance.length,
              present: allVolunteerAttendance.filter((a) =>
                ['present', 'halfDay'].includes(a.status)
              ).length,
            },
          },
