...
import { Plus, Edit2, Trash2, Key, Settings, Calendar, Clock, ChevronDown, ChevronUp, Link, User, CheckCircle, AlertCircle, X } from 'lucide-react';
import { classesAPI, usersAPI, settingsAPI, teachersAPI } from '../services/api';
...
export function ClassesPage() {
  ...
  const { data: classes, isLoading } = useQuery({
    queryKey: ['classes'], queryFn: () => classesAPI.getAll(),
    select: d => d.data?.data || [],
  });

  const createMutation = useMutation({ mutationFn: classesAPI.create, onSuccess: () => { qc.invalidateQueries(['classes']); toast.success('Class created'); setShowForm(false); setForm({ name: '', category: '', capacity: 30, description: '' }); }, onError: err => toast.error(err.response?.data?.message || 'Failed') });
  ...
  const deleteMutation = useMutation({ mutationFn: classesAPI.delete, onSuccess: () => { qc.invalidateQueries(['classes']); toast.success('Class deleted'); }, onError: err => toast.error(err.response?.data?.message || 'Failed') });
  ...
  return (
    <div>
          ...
          <motion.div key={cls._id} className="card" whileHover={{ y: -2 }} transition={{ duration: 0.15 }}>
            ...
            <div className="card-body">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 14 }}>
                {[
                  { label: 'Grade Range', value: cls.gradeRange || '—' },
                  { label: 'Students', value: `${cls.studentCount || 0} / ${cls.capacity}` },
                  { label: 'Teacher', value: cls.teacher?.name || 'Unassigned' },
                ].map(item => (
                  <div key={item.label}>
                    <div style={{ fontSize: '0.68rem', textTransform: 'uppercase', letterSpacing: '0.06em', color: 'var(--color-text-muted)', fontWeight: 600, marginBottom: 2 }}>{item.label}</div>
                    <div style={{ fontWeight: 600, fontSize: '0.85rem', color: item.label === 'Teacher' && !cls.teacher ? 'var(--color-text-muted)' : 'inherit' }}>{item.value}</div>
                  </div>
                ))}
              </div>
      ...
      <AnimatePresence>
        {(showForm || editClass) && (
          <div className="modal-overlay" onClick={() => { setShowForm(false); setEditClass(null); }}>
            <motion.div className="modal" onClick={e => e.stopPropagation()} initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }}>
              <div className="modal-header">
                ...
                <button className="btn btn-ghost btn-icon" onClick={() => { setShowForm(false); setEditClass(null); }}><X size={18} /></button>
              </div>
              <div className="modal-body">
                <div className="form-grid">
                  ...
                  <div className="form-group"><label className="form-label">Category <span className="required">*</span></label>
                    ...
                    {form.category && <div className="form-hint">Grade range: {form.category === 'Beginner' ? 'PreKG–Grade 2' : form.category === 'Primary' ? 'Grade 3–5' : form.category === 'Junior' ? 'Grade 6–8' : 'Grade 9–12'}</div>}
                  </div>
              ...
              <div className="modal-footer">
                <button className="btn btn-secondary" onClick={() => { setShowForm(false); setEditClass(null); }}>Cancel</button>
                <button className="btn btn-primary" onClick={() => editClass ? updateMutation.mutate({ id: editClass._id, data: form }) : createMutation.mutate(form)}>
...
export function UsersPage() {
  ...
  const createMutation = useMutation({ mutationFn: usersAPI.create, onSuccess: () => { qc.invalidateQueries(['users']); toast.success('User created successfully'); setShowForm(false); setForm({ userID: '', password: '', role: 'editor', name: '', email: '' }); }, onError: err => toast.error(err.response?.data?.message || 'Failed') });
  const toggleMutation = useMutation({ mutationFn: ({ id, isActive }) => usersAPI.update(id, { isActive }), onSuccess: () => { qc.invalidateQueries(['users']); toast.success('User updated'); } });
  const resetMutation = useMutation({ mutationFn: ({ id, password }) => usersAPI.resetPassword(id, { newPassword: password }), onSuccess: () => { qc.invalidateQueries(['users']); toast.success('Password reset successfully'); setResetTarget(null); setNewPassword(''); } });
            ...
            <motion.div className="modal" onClick={e => e.stopPropagation()} initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className="modal-body">
                <div className="form-grid">
                  ...
                  <div className="form-group"><label className="form-label">Username <span className="required">*</span></label><input className="form-input" value={form.userID} onChange={e => setForm({ ...form, userID: e.target.value })} placeholder="Login username" /></div>
                  ...
                  <div className="form-group"><label className="form-label">Temporary Password <span className="required">*</span></label><input className="form-input" type="password" value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder="Min 8 characters" /></div>
                ...
                <div className="alert alert-warning mt-3"><AlertCircle size={15} /><div>User will be prompted to change their password on first login from Settings menu.</div></div>
              </div>
              <div className="modal-footer"><button className="btn btn-secondary" onClick={() => setShowForm(false)}>Cancel</button><button className="btn btn-primary" onClick={() => createMutation.mutate(form)}>Create User</button></div>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
        ...
        {resetTarget && (
          <div className="modal-overlay" onClick={() => setResetTarget(null)}>
            <motion.div className="modal modal-sm" onClick={e => e.stopPropagation()} initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className="modal-body">
                <div className="form-group"><label className="form-label">New Temporary Password</label><input className="form-input" type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder="Min 8 characters" /></div>
              </div>
              <div className="modal-footer"><button className="btn btn-secondary" onClick={() => setResetTarget(null)}>Cancel</button><button className="btn btn-primary" onClick={() => resetMutation.mutate({ id: resetTarget._id, password: newPassword })} disabled={!newPassword || newPassword.length < 8}>Reset Password</button></div>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
...
export function SettingsPage() {
  ...
  const { data: allSettings, isLoading } = useQuery({
    queryKey: ['all-settings'], queryFn: () => settingsAPI.getAll(), select: d => d.data?.data || [],
  });

  const createMutation = useMutation({ mutationFn: settingsAPI.create, onSuccess: () => { qc.invalidateQueries(['all-settings']); toast.success('VBS year created'); closeModal(); } });
  const updateMutation = useMutation({ mutationFn: ({ id, data }) => settingsAPI.update(id, data), onSuccess: () => { qc.invalidateQueries(['all-settings']); toast.success('Settings updated'); closeModal(); } });
  const activateMutation = useMutation({ mutationFn: settingsAPI.activate, onSuccess: () => { qc.invalidateQueries(['all-settings']); toast.success('Year activated'); } });

  const openCreate = () => {
    setEditSettings(null);
    setForm({
      year: new Date().getFullYear(), vbsTitle: '', tagline: '',
      startDate: '', endDate: '',
  ...
  const addTheme = () => {
    const day = dailyThemes.length + 1;
    setDailyThemes(prev => [...prev, { day, title: '', verse: '', verseText: '', description: '' }]);
  };
  ...
  return (
    <div>
      <div className="page-header">
        <div><h1 className="page-title">VBS Settings</h1><p className="page-subtitle">Configure VBS years, themes, attendance windows</p></div>
        <button className="btn btn-primary" onClick={openCreate}><Plus size={16} /> New VBS Year</button>
      </div>

      {/* Change Password Card — always visible in Settings */}
        ...
        <div className="card-header">
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.78rem', color: 'var(--color-text-secondary)' }}>Update your account password</div>
            </div>
          </div>
              ...
              <div className="card-body">
                <div className="form-grid" style={{ maxWidth: 520 }}>
                  ...
                  <div className="form-group">
                    ...
                    <input className="form-input" type="password" value={pwForm.newPassword} onChange={e => setPwForm({ ...pwForm, newPassword: e.target.value })} placeholder="Min 8 characters" />
                  </div>
                  <div className="form-group">
            ...
            <div className="card-header">
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  {s.isActive && <span className="badge badge-green" style={{ marginTop: 4 }}>ACTIVE</span>}
                </div>
              </div>
          ...
          <div className="modal-overlay" onClick={closeModal}>
            <motion.div className="modal modal-xl" onClick={e => e.stopPropagation()} initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}>
              ...
              <div style={{ padding: '0 22px', borderBottom: '1px solid var(--color-border)', display: 'flex', gap: 4 }}>
                {[
                  ...
                  <button key={sec.id} onClick={() => setActiveSection(sec.id)}
                    style={{ padding: '10px 14px', border: 'none', background: 'none', cursor: 'pointer', fontWeight: 600, fontSize: '0.82rem', fontFamily: 'var(--font-sans)', color: activeSection === sec.id ? 'var(--color-primary)' : 'var(--color-text-secondary)', borderBottom: activeSection === sec.id ? '2px solid var(--color-primary)' : '2px solid transparent', marginBottom: -1 }}>
                    {sec.label}
                  </button>
                ))}
              </div>
              ...
              <div className="modal-body">
                ...
                {activeSection === 'dates' && (
                  <div>
                    <div className="alert alert-info mb-3"><AlertCircle size={15} /><div>Attendance window is the time period during which teachers can submit student attendance each day. Outside this window, only admins can submit.</div></div>
                    <div className="form-grid">
                      ...
                      <div className="form-group"><label className="form-label">Attendance Window Opens (IST) <span className="required">*</span></label><input className="form-input" type="time" value={form.timeWindowStart || '08:00'} onChange={e => setForm({ ...form, timeWindowStart: e.target.value })} /></div>
                      <div className="form-group"><label className="form-label">Attendance Window Closes (IST) <span className="required">*</span></label><input className="form-input" type="time" value={form.timeWindowEnd || '10:00'} onChange={e => setForm({ ...form, timeWindowEnd: e.target.value })} /></div>
                    </div>
                ...
                {activeSection === 'theme' && (
                  <div className="form-grid">
                    <div className="form-group"><label className="form-label">Primary Color</label><div style={{ display: 'flex', gap: 10, alignItems: 'center' }}><input type="color" value={form.mainColor || '#1a2f5e'} onChange={e => setForm({ ...form, mainColor: e.target.value })} style={{ width: 44, height: 38, border: '1.5px solid var(--color-border)', borderRadius: 8, cursor: 'pointer', padding: 2 }} /><input className="form-input" value={form.mainColor || '#1a2f5e'} onChange={e => setForm({ ...form, mainColor: e.target.value })} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.85rem' }} /></div></div>
                    <div className="form-group"><label className="form-label">Accent Color</label><div style={{ display: 'flex', gap: 10, alignItems: 'center' }}><input type="color" value={form.accentColor || '#c8922a'} onChange={e => setForm({ ...form, accentColor: e.target.value })} style={{ width: 44, height: 38, border: '1.5px solid var(--color-border)', borderRadius: 8, cursor: 'pointer', padding: 2 }} /><input className="form-input" value={form.accentColor || '#c8922a'} onChange={e => setForm({ ...form, accentColor: e.target.value })} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.85rem' }} /></div></div>
                    <div style={{ gridColumn: '1/-1', padding: 16, borderRadius: 12, border: '1px solid var(--color-border)', background: form.mainColor || '#1a2f5e', color: 'white', textAlign: 'center' }}>
                      ...
                      <div style={{ marginTop: 12, display: 'inline-block', background: form.accentColor || '#c8922a', color: 'white', padding: '4px 16px', borderRadius: 99, fontSize: '0.8rem', fontWeight: 700 }}>Accent Color</div>
                    </div>
                  </div>
                )}
                ...
                {activeSection === 'daily' && (
                  <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      ...
                      {dailyThemes.length === 0 && <div style={{ textAlign: 'center', padding: 32, color: 'var(--color-text-muted)', fontSize: '0.85rem' }}>No daily themes configured yet. Click "Add Day 1" to start.</div>}
                    </div>
                  </div>
                )}
              </div>
