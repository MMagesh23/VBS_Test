...
export function ClassesPage() {
  ...
  const closeModal = () => {
    setShowForm(false);
    setEditClass(null);
    setForm({ name: '', category: '', capacity: 30, description: '' });
  };
              ...
              {(users || []).map(u => (
                <tr key={u._id}>
                  ...
                  <td>
                    {u.role !== 'admin' && (
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button className="btn btn-secondary btn-sm" onClick={() => handleDeactivate(u)} disabled={toggleLoading}>
                          {u.isActive ? 'Deactivate' : 'Activate'}
                        </button>
                        <button className="btn btn-secondary btn-icon btn-sm" onClick={() => setResetTarget(u)} title="Reset password">
                          <Key size={13} />
                        </button>
                      </div>
                    )}
...
export function SettingsPage() {
  ...
  const activateMut = useMutation({
    ...
    onSuccess: (_, id) => {
      qc.invalidateQueries(['all-settings']);
      refreshYears();
  ...
  const handleActivateConfirm = async s => {
    ...
    const ok = await confirm({
      ...
      message: `"${s.vbsTitle}" will become the active year. All data views will switch to ${s.year}.`,
      confirmLabel: `Activate ${s.year}`,
      type: 'info',
    });
          ...
          <div key={s._id} className="card">
            <div className="card-header">
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {!s.isActive && (
                  <button className="btn btn-success btn-sm" onClick={() => handleActivateConfirm(s)} disabled={activateLoading}>
                    Set Active
                  </button>
                )}
