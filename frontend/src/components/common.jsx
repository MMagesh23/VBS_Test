      ...
      <div className="modal-overlay" onClick={onClose}>
        ...
        >
          <div className="modal-header">
            ...
            <button
              onClick={onClose}
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', display: 'flex', alignItems: 'center', padding: 4, borderRadius: 6 }}
            >
              <X size={18} />
            </button>
          </div>
...
export function Alert({ type = 'info', children, style }) {
  ...
  return (
    <div style={{
      ...
      color: s.color, fontSize: '0.875rem', lineHeight: 1.5,
      ...style,
    }}>
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        ...
        {Array.from({ length: pages }, (_, i) => i + 1)
          ...
          .map((p, i) =>
            p === '...' ? (
              <span key={`ellipsis-${i}`} style={{ padding: '6px 4px', color: 'var(--color-text-muted)', fontSize: '0.8rem' }}>…</span>
            ) : (
              <button
                key={p}
    ...
    <div className="stat-card">
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
        <div>
          ...
          {subtitle && (
            <div style={{ fontSize: '0.75rem', color: 'var(--color-text-muted)', marginTop: 6, fontWeight: 500 }}>
              {subtitle}
            </div>
          )}
        </div>
