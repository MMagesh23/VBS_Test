...
function TeacherAttendanceHistory() {
  ...
  const { data: history, isLoading } = useQuery({
    ...
    // FIX: API returns descending; sort ascending so Day 1 = first/earliest VBS date
    queryFn: () => attendanceAPI.getStudentAttendance({ classId: classData._id })
      .then(r => [...(r.data?.data || [])].sort((a, b) => new Date(a.date) - new Date(b.date))),
    enabled: !!classData?._id,
  });
