...
// @desc    Student analytics — updated for new fields
    ...
    const [studentTrends, teacherTrends, volunteerTrends, classPerformance] = await Promise.all([
      StudentAttendance.aggregate([
        ...
        { $project: {
          ...
          rate: { $cond: [{ $gt: [{ $add: ['$present', '$absent'] }, 0] },
            { $multiply: [{ $divide: ['$present', { $add: ['$present', '$absent'] }] }, 100] }, 0] },
        }},
      ]),
      ...
      StudentAttendance.aggregate([
        ...
        { $project: {
          ...
          attendanceRate: { $cond: [{ $gt: ['$totalRecords', 0] },
            { $multiply: [{ $divide: ['$totalPresent', '$totalRecords'] }, 100] }, 0] },
